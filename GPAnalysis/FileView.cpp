...
void  CFileView::LoadDirectory(CString strDir, HTREEITEM hParent)
{
	CFileFind finder;
	BOOL      bWorking;
	CString   strName;
	
	// For find all folders
	if( strDir.Right(1) == "\\" )
	...
	while (bWorking)
	{
		...
		if (strName.GetAt(0) == '.') continue;

		if (finder.IsDirectory())
		{
			HTREEITEM hDir = m_wndFileView.InsertItem(finder.GetFileName(), 0, 0, hParent);
			LoadDirectory(finder.GetFilePath(), hDir);
		}
		//else
		//{
		//	m_wndFileView.InsertItem(finder.GetFileName(), 1, 1, hParent);
		//}
	}
	finder.Close();
	...
	while (bWorking)
	{
		...
		if (!finder.IsDirectory())
		{
			m_wndFileView.InsertItem(finder.GetFileName(), 2, 2, hParent);
		}
	}
	finder.Close();
}
...
{
	CString strRoot = gSetting.m_DataSourcePath;

	HTREEITEM hRoot = m_wndFileView.InsertItem(_T("/"), 0, 0);
	m_wndFileView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	LoadDirectory(strRoot, hRoot);
